...
Touches: `src/Editor/FrameDebuggerData.h/.cpp` (new formatting helpers +
...
- `FrameDebuggerEventDetails` (PHASE1) already has every field this
  section needs: `shaderName`, `passName`, `blendMode`, `zClip`, `zTest`,
  `zWrite`, `cull`, `stencilRef`, `stencilComp`, `stencilPass`,
  `stencilFail`, `stencilZFail`, `textures`, `vectors`, `matrices`.
...
- `ImGui::BeginTabBar("...")`/`ImGui::BeginTabItem("...")`/`EndTabItem()`/
  ...
  nowhere else in this codebase yet, so this is this engine's first tab
  bar; keep it simple (two fixed tabs, "Preview" and "ShaderProperties",
  no closable/reorderable flags needed).
...
- Formatting a `FrameDebuggerVectorProperty`/`FrameDebuggerMatrixProperty`
  ...
  helpers' *shape*, but promote these two into the genuinely-shared,
  already-Tier-1-tested `FrameDebuggerData.h/.cpp` (PHASE1's file) instead
  of a panel-local anonymous-namespace helper, specifically because they
  operate on pure data with no ImGui dependency at all and deserve real
  unit tests (`RenderGraphPanel.cpp`'s own helpers are NOT unit-tested,
  because they live in a Tier-2 `.cpp` file, not a Tier-1 one — this
  phase's own helpers should NOT repeat that gap, since
  `FrameDebuggerData.h/.cpp` already is that Tier-1 file).

## Step 3: The Plan

### 3.1 `FrameDebuggerData.h` — two new pure formatting functions

Append, after `FindEventDetailsByIndex()`'s own declaration:
...
- `FormatMatrixPropertyTest`: hand-build a
  ...
  result.end(), '\n') == 3` (4 rows) is sufficient and robust).
...
void FrameDebuggerPanel::BuildEventDetailsSection(const std::optional<FrameDebuggerEventDetails>& details)
{
    ...
    ImGui::Text("Event #%d: %s", d.eventIndex, d.shaderName.c_str());
    ImGui::Separator();
...
(NOTE: this document uses `#` at the start of a couple of comment-body
lines above purely to keep this Markdown file's own code fences from
being misinterpreted — when actually writing the `.cpp` file, use a
normal `//` C++ comment prefix throughout, not a literal `#`.)
