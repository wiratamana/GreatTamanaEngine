# Phase 4 — Vulkan GPU Timestamp Queries — Implementation Strategy (v2)

Status: PROPOSAL / PLANNING DOCUMENT — no implementation yet.

## Changelog: v1 -> v2

This revision was produced by re-reading v1 end-to-end against the actual
engine source (and the attached `Phase4.md` outline) a second time, looking
specifically for (a) missing context v1 didn't account for, and (b) places
where v1's own design could be improved before any code is written. Nothing
about the four-sub-phase structure (4A -> 4B -> 4C -> 4D), the ordering, or
the overall "reuse existing synchronization, never add a new wait"
philosophy changed — those were already correct. Three concrete,
load-bearing findings did come out of this pass, and are folded directly
into the plan below (not just noted here):

1. **v1 never once mentions `GTE_ENABLE_PROFILER` or
   `FrameProfiler::IsCaptureEnabled()`.** Every other piece of this engine's
   profiling instrumentation (`ScopeTimer`'s compiled-out-when-`OFF` body;
   `DrawStats.h`'s own comment explaining precisely why IT is the one
   exception that does NOT need this gate) explicitly reasons about the
   two-layer compile-time/runtime on/off switch AGENTS.md's "Profiling"
   section establishes as a standing convention. v1's `GpuTimingService`
   design instead unconditionally issued `vkCmdResetQueryPool`/
   `vkCmdWriteTimestamp2`/`vkGetQueryPoolResults` every single frame,
   regardless of whether the Editor's "Profiler" panel even exists, whether
   its "Capture" toggle is off, or whether the whole engine was built with
   `-DGTE_ENABLE_PROFILER=OFF`. Unlike `DrawStats.h`'s CPU accumulator
   (which rides for free on an iteration that already has to happen every
   frame for unrelated reasons — real draw submission — and is explicitly,
   correctly, NOT gated because of that), a GPU timestamp query is NOT
   free: it is additional recorded GPU work plus a real driver call to read
   results back. v2 adds an explicit, narrow two-layer gate (Step 2.3,
   Phase 4B) that keeps `Renderer`/`FramePresenter`/`GpuTimingService`
   completely free of any `Profiling/` header (preserving this document's
   own pre-existing decoupling decision) while still letting the actual
   Vulkan work be skipped both at compile time (`GTE_ENABLE_PROFILER=OFF`)
   and at runtime (the Editor's existing "Capture" checkbox).
2. **v1's new, no-default `GpuTimingSlot` parameter on
   `Renderer::RenderOffscreen()` silently introduced a real slot-collision
   bug for every OTHER existing caller of that function.**
   `RenderOffscreen()` is not exclusively called from `Application::Run()`'s
   Game/Scene blocks — `AssetPreviewMesh` (and, sharing the same
   preview/shader pattern, very likely `BoneViewerWindow` too — see
   `README.md`, "Editor / Debug UI": "`AssetPreviewMesh` builds its own
   `VkPipeline`/`VkPipelineLayout` directly and records its own indexed
   draw call via a `Renderer::RenderOffscreen()` `recordExtra` callback")
   already calls it, for the Inspector's live mesh preview and the Bone
   Viewer's own 3D viewport. v1's plan, as written, would force whoever
   implements it to pick SOME `GpuTimingSlot` value at those call sites
   just to make the build compile again — and since only two offscreen
   slots exist (`Offscreen0`/`Offscreen1`), picking either one would make
   an Inspector mesh preview (or the Bone Viewer) silently overwrite, and
   get misreported as, "Game View"/"Scene View" GPU timing the instant a
   user clicks a Mesh asset in "Project" or opens the Bone Viewer — a real
   data-integrity bug, not a cosmetic one. v2 fixes this by changing the
   new parameter's type to `std::optional<GpuTimingSlot>`, where
   `std::nullopt` means "do not instrument this call at all" (zero Vulkan
   query traffic for it, no slot touched) — see Step 2.2/2.3, "Overall API
   surface", and Phase 4C below.
3. **v1 never explicitly wrote down the Vulkan validity constraint that
   `vkCmdResetQueryPool` must be recorded OUTSIDE a dynamic rendering
   instance** (never between a `vkCmdBeginRendering`/`vkCmdEndRendering`
   pair). v1's actual call-site placement already happens to satisfy this
   — every reset/write this phase adds is recorded by `FramePresenter`
   itself, bracketing its own call to `FrameRecorder::RecordFrame()` (the
   ONE place that ever calls `vkCmdBeginRendering`/`vkCmdEndRendering`),
   never threaded into `RecordFrame()`'s own body — but that was true by
   ACCIDENT of where the calls were placed, not because anyone had written
   the rule down. Made explicit in Step 2.3, so a future edit tempted to
   move one of these calls "closer to what it's measuring" doesn't
   introduce a validation error it doesn't understand.

Everything else below is v1's own reasoning, materially unchanged — this
remains "why before what" first, grounded in the same direct reading of the
current source `PROFILER_IMPLEMENTATION_STATUS_v6.md` describes as already
done for phases 0/1/2/3/5/7.
...
- **[v2]** This is delivered without ever paying for the measurement when
  nobody asked for it: turning the Editor's existing "Capture" toggle off,
  or building with `-DGTE_ENABLE_PROFILER=OFF`, must mean genuinely zero
  new `vkCmd*`/`vkGetQueryPoolResults` calls per frame, not merely "the
  Profiler panel stops displaying a number it's still secretly computing."
  See Step 2.3/Phase 4B.
- **[v2]** This is delivered without corrupting the Editor's OWN debug
  tooling: an Inspector mesh preview or an open Bone Viewer window (both of
  which already call `Renderer::RenderOffscreen()` today, for reasons
  entirely unrelated to the Profiler's three named passes) must never
  silently share a query slot with, and therefore never silently corrupt,
  "Game View"/"Scene View" GPU timing. See Step 2.2/2.3 and Phase 4C.
...
| **4B** | A real, RAII-owned `VkQueryPool` sized for every pass/path, wired into `Renderer`'s ownership graph, and gated by the engine's existing two-layer profiler on/off switch | Normal rendering (Editor open, validation layers on) is completely unaffected — the pool exists and is correctly reset, but nothing reads from it or reports to the Profiler yet; a `GTE_ENABLE_PROFILER=OFF` build never creates the pool at all. |
| **4C** | Game View + Scene View GPU timing, using the already-synchronous offscreen fence, with an explicit opt-out for non-Profiler-tracked offscreen callers | The "Profiler" panel shows real GPU milliseconds for whichever of "Game"/"Scene" is currently visible, and an honest `N/A` for whichever is hidden — never `0.00 ms` — and the Inspector's mesh preview / Bone Viewer render exactly as before, untouched by any of this. |
...
- **[v2] Turning Capture off genuinely stops the Vulkan work, not just the
  display.** With the Editor's "Capture" checkbox unticked, a RenderDoc/
  validation-layer capture of a few frames shows no new
  `vkCmdResetQueryPool`/`vkCmdWriteTimestamp2` commands at all in either
  the offscreen or Present command buffers — verified manually alongside
  the resize/minimize checklist in Phase 4D.
- **[v2] The Editor's own debug-preview tooling is provably unaffected.**
  With an Inspector mesh selected (or the Bone Viewer open) WHILE "Game"/
  "Scene" are both visible and animating, the Profiler panel's Game
  View/Scene View GPU Timing values never glitch, freeze, or jump when the
  preview is opened/closed/changed — proving the opt-out (Step 2.2/2.3,
  Phase 4C) actually isolates the two.
...
- **`src/Renderer/DrawStats.h` is the direct structural precedent for how
  ...
  Vulkan headers" shape. **`DrawStats.h`'s own comment ALSO explicitly
  explains why IT is deliberately NOT gated behind `GTE_ENABLE_PROFILER`
  — "`m_drawQueue` is already iterated unconditionally every frame to
  issue the real draw calls regardless of that switch, so this
  accumulation rides along on that same, already-necessary iteration at
  effectively no extra measurable cost." This reasoning does NOT transfer
  to GPU timestamp queries** — there is no pre-existing, unconditional
  per-frame Vulkan call this phase's work could "ride along" on for free;
  every `vkCmdResetQueryPool`/`vkCmdWriteTimestamp2`/
  `vkGetQueryPoolResults` this phase adds is new, additional GPU/driver
  work that exists ONLY because this feature exists. That is precisely
  why Phase 4, unlike Phase 3, needs its own explicit on/off gate — see
  Step 2.3 below.
...
- **[v2] `Renderer::RenderOffscreen()` has MORE callers than just
  `Application.cpp`'s Game/Scene blocks, and none of them have any business
  being associated with either of the Profiler's two named offscreen
  passes.** Confirmed by direct reading of `README.md`'s "Editor / Debug
  UI" section: `AssetPreviewMesh` (the Inspector's live 3D mesh preview,
  `src/Editor/AssetPreviewMesh.cpp`) "records its own indexed draw call
  via a `Renderer::RenderOffscreen()` `recordExtra` callback" — and the
  Bone Viewer (`BoneViewerWindow`, which explicitly reuses
  `AssetPreviewMesh`'s own shader pair and rendering approach) is very
  likely built the exact same way (to be confirmed by grep at
  implementation time, same as v1 already flagged for the general "every
  `RenderOffscreen(` call site" sweep — see Phase 4C). If Phase 4's new
  `GpuTimingSlot` parameter has no way to say "this call has nothing to do
  with the Profiler's three named passes, don't touch a query slot for
  it," whoever implements this will be forced to pick `Offscreen0` or
  `Offscreen1` just to satisfy the compiler — silently making a Mesh-asset
  preview or a Bone Viewer frame overwrite, and get reported as, "Game
  View"/"Scene View" GPU timing. This is a genuine correctness gap in a
  naive reading of the plan, not merely an inconvenience — fixed in Step
  2.3/Phase 4C by making the new parameter `std::optional<GpuTimingSlot>`.
- **[v2] There is no discussion anywhere (until this revision) of whether
  this phase's actual Vulkan recording work should respect the engine's
  existing two-layer profiling on/off switch.** AGENTS.md's "Profiling"
  section establishes, as a standing, already-shipped convention, that
  CPU-side instrumentation has BOTH a compile-time switch
  (`GTE_ENABLE_PROFILER`, making `ScopeTimer`'s body a true empty no-op
  when off) and a runtime switch
  (`FrameProfiler::SetCaptureEnabled()`/`IsCaptureEnabled()`, skipping the
  clock read on an already-compiled-in `ScopeTimer`). Nothing in v1
  addressed whether Phase 4's Vulkan query recording — which, unlike
  `DrawStats.h`'s CPU accumulation, is NOT free (see 2.1's new bullet
  above) — should honor either layer. Left unaddressed, this phase would
  keep issuing real GPU commands and driver read-back calls every single
  frame even with the Editor's "Capture" checkbox unticked, or even in a
  deliberately minimal `GTE_ENABLE_PROFILER=OFF` build — quietly
  contradicting the "genuinely zero cost when off" philosophy this
  codebase otherwise applies consistently. Fixed in Step 2.3/Phase 4B.
...
- **`FrameRecorder` must NOT gain any awareness of query pools, GPU
  ...
  after this phase's own small API addition, WHICH offscreen slot, if
  any) has any business knowing. This mirrors exactly why `DrawStats`
  counting is fused INLINE into `RecordFrame()`'s existing per-item draw
  loop (a correctness requirement, per `DrawStats.h`'s own comment) while
  GPU timestamp writes are NOT — the two are fundamentally different in
  kind: draw-call counting is a per-item accumulation that must stay
  perfectly synchronized with what's actually drawn, item by item, inside
  the loop `RecordFrame()` already owns; a GPU timestamp pair is a single
...
- **[v2] `vkCmdResetQueryPool` must be recorded OUTSIDE a dynamic rendering
  instance — never between a `vkCmdBeginRendering`/`vkCmdEndRendering`
  pair.** This is a hard Vulkan validity rule (`vkCmdWriteTimestamp2` is,
  by contrast, legal either inside or outside one — but there is no
  benefit to placing it inside here, and doing so would only make the
  bracket harder to reason about). The placement this whole plan already
  specifies — every reset/write call recorded by `FramePresenter` itself,
  bracketing its own call to `FrameRecorder::RecordFrame()` (the ONE place
  that ever calls `vkCmdBeginRendering`/`vkCmdEndRendering`), never
  threaded into `RecordFrame()`'s own body — already satisfies this rule
  by construction (see the constraint immediately above). This was true by
  ACCIDENT of where the calls were already being placed for an unrelated
  reason (keeping `FrameRecorder` state-free); it had never actually been
  written down as its own rule until this revision. Write a short comment
  to this effect at each of the four call sites
  (`RecordOffscreenPassStart`/`End`, `RecordPresentPassStart`/`End`'s own
  call sites in `FramePresenter.cpp`) once implemented, so a future edit
  that's tempted to move one of these calls "closer to what it's actually
  measuring" (e.g. into `FrameRecorder::RecordFrame()`, between its own
  `vkCmdBeginRendering`/`vkCmdEndRendering`) understands exactly why that
  would be both a layering violation (the constraint above) AND a Vulkan
  validation error (this constraint), not just a style preference.
...
- **[v2] Phase 4's actual Vulkan query recording must respect the same
  two-layer profiling on/off switch every other piece of this engine's
  profiling instrumentation already respects (AGENTS.md, "Profiling") —
  while still keeping `Renderer`/`FramePresenter`/`GpuTimingService`
  completely free of any `Profiling/` header (this document's own
  pre-existing, unchanged design decision — see the design decision log
  below).**
  - **Compile-time layer.** `GpuTimingService`'s constructor is written so
    that, when `GTE_ENABLE_PROFILER` is `0`, it behaves EXACTLY as if the
    device reported `GpuTimestampCapability::supported == false` —
    `m_queryPool` is never constructed and no `VkQueryPool` is ever
    created, regardless of what `VulkanDevice::TimestampCapability()`
    itself actually reports. This is `GpuTimingService`'s own one, narrow
    `#if GTE_ENABLE_PROFILER`, isolated to its constructor body — every
    call site in `FramePresenter`/`Renderer` stays unconditional (they
    always call into the service; the service itself is simply inert when
    this switch is off), mirroring `ScopeTimer`'s own "genuinely zero cost
    when off" compiled-out branch (`ScopeTimer.h`'s `#else` form).
  - **Runtime layer.** `GpuTimingService` gains
    `void SetCaptureEnabled(bool enabled) noexcept` (mirroring
    `FrameProfiler::SetCaptureEnabled()`'s own name/shape exactly, on
    purpose), and every one of its `Record*`/`Read*` methods early-returns
    (no Vulkan call at all) whenever this flag is `false` — the exact same
    style of early-return the `!IsSupported()` check already uses. This is
    what lets the Editor's EXISTING "Capture" checkbox
    (`Profiling::FrameProfiler::SetCaptureEnabled()`, wired up since Phase
    7) also genuinely stop this phase's Vulkan work while it's off, without
    needing a second, Profiler-panel-specific control.
  - **The bridge stays exactly where every other Renderer<->Profiling
    bridge already lives: `Application.cpp`.** `Renderer` gains
    `void SetGpuTimingCaptureEnabled(bool enabled) noexcept { m_gpuTiming->
    SetCaptureEnabled(enabled); }` — taking a PLAIN `bool`, never a
    `Profiling::`-namespaced type, so `Renderer` itself still never
    includes a single `Profiling/` header. `Application::Run()` calls this
    once per frame, before the first `RenderOffscreen()`/`Present()` call
    of that frame:
    `m_renderer.SetGpuTimingCaptureEnabled(Profiling::FrameProfiler::
    Instance().IsCaptureEnabled());` — a single, cheap bool read/forward,
    exactly the same shape as every OTHER Renderer-plain-data-into-
    `Profiling::`-typed-call bridge this document's own design decision log
    already establishes (`DrawStats` -> `SetGpuPassDrawStats()`,
    `GpuTimingSample` -> `SetGpuPassTiming()`) — not a new architectural
    seam, an application of the existing one in the other direction.
  - **What a caller observes while capture is off:** `Read*`/
    `LastKnown()` return `GpuTimingSample{ Status::Absent, 0.0 }` — the
    SAME status a not-yet-warmed-up Present slot or a hidden panel already
    reports — via the same early-return path, never a stale value left
    over from before Capture was disabled, and never `Unsupported` (which
    specifically, and only, means "this device/build can never produce
    this measurement," a different, permanent condition Capture-toggling
...
- **`gte::GpuTimingSlot`** (enum class, Phase 4A) — `Offscreen0 = 0`,
  ...
  `FrameRecorder` decoupling rule, applied one layer up here too). The
  enum itself is unchanged from v1 — see the very next bullet for how v2
  actually closes the "what about a caller that means neither of these"
  gap, which is at the call-boundary, not inside the enum.
- **[v2] `Renderer::RenderOffscreen()`'s timing parameter is
  `std::optional<GpuTimingSlot>`, not a bare `GpuTimingSlot`.** A caller
  passes `GpuTimingSlot::Offscreen0`/`Offscreen1` to participate in Phase
  4's Profiler-visible GPU timing, or `std::nullopt` to explicitly opt
  OUT — meaning "record and submit this pass exactly as before, but do not
  touch a query slot, do not call into `GpuTimingService` at all, and do
  not affect `Renderer::LastGpuTiming()`'s cached values in any way."
  `std::nullopt` is still an explicit, required argument (no default) —
  same reasoning as v1's original "no silent default" rule, just extended
  to cover "no timing at all" as its own explicit choice rather than an
  implicit fallback. See Step 2.2/2.3 for why this exists (the
  `AssetPreviewMesh`/Bone Viewer slot-collision bug) and Phase 4C for the
...
- **`gte::GpuTimingService`** (class, Phase 4B, `src/Renderer/
  ...
  supported AND compiled in — see Step 2.3's new capture-gating
  constraint) plus the capability struct plus per-slot cached
  `GpuTimingSample` results, Present-path warm-up bookkeeping, and (v2) a
  runtime capture-enabled flag; the actual `vkCmdResetQueryPool`/
  `vkCmdWriteTimestamp2`/`vkGetQueryPoolResults` call sites live here,
  never inline in `FramePresenter.cpp` itself (keeping `FramePresenter`
  focused on ORCHESTRATING when to call these, not the raw Vulkan-query
  mechanics themselves — the same division of labor `FramePresenter`
  already has with `VulkanSwapchain`/`VulkanFrameSync`).
...
- **`Renderer::RenderOffscreen(RenderTexture& target, std::optional<
  GpuTimingSlot> timingSlot, const std::function<void(VkCommandBuffer)>&
  recordExtra = {})`** (Phase 4C) — ONE new parameter added to the
  existing method, telling `FramePresenter` which of `Offscreen0`/
  `Offscreen1` this particular call's GPU timing belongs to, or that it
  doesn't belong to either. Every existing call site must be updated to
  pass one (see Phase 4C).
...
- **[v2] `Renderer::SetGpuTimingCaptureEnabled(bool enabled) noexcept`**
  (Phase 4B) — a plain forwarding setter, taking a `bool` and nothing
  `Profiling`-typed, that `Application::Run()` calls once per frame with
  `Profiling::FrameProfiler::Instance().IsCaptureEnabled()`. See Step
  2.3's new capture-gating constraint above for the full reasoning.
...
- **A small, fixed indexing helper for the Present path's per-frame-in-
  ...
  - Total pool layout (documented once, here, as the single source of
    ...
    (**[v2]** Because a non-tracked `RenderOffscreen()` call now opts out
    via `std::nullopt` rather than being forced into one of these two
    slots, this fixed 8-slot layout remains exactly sufficient — it never
    needs to grow to accommodate `AssetPreviewMesh`/`BoneViewerWindow`,
    which simply never touch the pool at all.)
...
before any actual measurement logic touches it. **[v2] This sub-phase also
wires up the compile-time/runtime capture gate (Step 2.3) from the very
start, so 4C/4D never have to retrofit it onto an already-"always on"
service.**
...
- Constructor: `GpuTimingService(VkDevice device, const
  GpuTimestampCapability& capability)`. **[v2]** The FIRST thing this
  constructor does is decide the EFFECTIVE capability it will actually
  operate under:
  ```
  GpuTimestampCapability effective = capability;
  #if !GTE_ENABLE_PROFILER
  effective.supported = false;
  #endif
  ```
  — so a `GTE_ENABLE_PROFILER=OFF` build behaves identically to running on
  a device that reported no timestamp support at all, without needing a
  second, parallel "is this feature compiled in" flag threaded through
  every method below. If `effective.supported`, constructs
  `std::optional<VulkanQueryPool> m_queryPool` with `kGpuTimingSlotCount ==
  8` slots (Phase 4A's fixed layout) and performs an initial FULL reset of
  every slot via a one-shot command buffer (reuse
  ...
  branch scattered through their own logic. If `!effective.supported`
  (device-reported unsupported, OR `GTE_ENABLE_PROFILER=OFF`),
  `m_queryPool` stays empty (`std::nullopt`) and every other method below
  becomes a safe, cheap no-op/early-return — **and no `VkQueryPool` is
  ever created for the lifetime of the process in a
  `GTE_ENABLE_PROFILER=OFF` build**, matching the "genuinely zero cost, not
  just small" bar `ScopeTimer`'s own compiled-out branch already sets.
- Stores `GpuTimestampCapability m_capability` (the EFFECTIVE one, copied
  in — this is what `IsSupported()` below reports; a caller has no way to
  distinguish "device can't do this" from "build was configured not to,"
  which is intentional, since both mean exactly the same thing to every
  consumer of this class: no real data is available, ever, this session).
- **[v2]** Stores `bool m_captureEnabled = true;` — the runtime layer of
  the gate. Note this is INDEPENDENT of `m_capability.supported`: a
  capable, compiled-in service can still have its recording temporarily
  suppressed at runtime via `SetCaptureEnabled(false)`, exactly mirroring
  `FrameProfiler`'s own `m_captureEnabled` field and its interaction with
  `BeginFrame()`/`RecordCpuScope()`/etc.
...
- **Public methods, called from `FramePresenter`:**
  - `bool IsSupported() const noexcept` — trivial passthrough of
    `m_capability.supported` (the EFFECTIVE, post-`GTE_ENABLE_PROFILER`
    value); every call site below is expected to check this first (or the
    methods themselves early-return safely if not — both are fine;
    recommend the methods themselves guard internally, so
  ...
  - **[v2]** `void SetCaptureEnabled(bool enabled) noexcept` — sets
    `m_captureEnabled`. Called from `Renderer::SetGpuTimingCaptureEnabled()`
    (see the "Overall API surface" section above), which
    `Application::Run()` calls once per frame from
    `Profiling::FrameProfiler::Instance().IsCaptureEnabled()`.
  - `void RecordOffscreenPassStart(VkCommandBuffer cmd, GpuTimingSlot
    slot) noexcept` — no-op if `!IsSupported() || !m_captureEnabled`;
    otherwise records `vkCmdResetQueryPool(cmd, pool, startSlotFor(slot),
    2)` (resetting BOTH the start and end slot for this pass in one call —
    legal, and simpler than two separate reset calls) then
  ...
  - `void RecordOffscreenPassEnd(VkCommandBuffer cmd, GpuTimingSlot slot)
    noexcept` — no-op under the same conditions; otherwise
  ...
  - `GpuTimingSample ReadOffscreenResultNow(GpuTimingSlot slot) noexcept`
    ...
    fence wait) that the corresponding submission is complete. If
    `!IsSupported()`, returns (and caches) `GpuTimingSample{
    Status::Unsupported, 0.0 }` without touching Vulkan at all. **[v2]**
    Else if `!m_captureEnabled`, returns (and caches) `GpuTimingSample{
    Status::Absent, 0.0 }` without touching Vulkan at all — capture being
    off is a temporary, "no data available right now" condition, not a
    permanent "this can never work" one, so it must map to `Absent`, never
    `Unsupported`. Otherwise calls `vkGetQueryPoolResults(...,
    VK_QUERY_RESULT_64_BIT)` (WAIT_BIT not required — see Step 2.1 — but
    harmless to include defensively) for the 2-slot range, converts via
    `ConvertTimestampDeltaToMilliseconds()` (Phase 4A), builds a
    `GpuTimingSample{ Status::Present, ms }`, stores it into
    `m_lastKnown[...]`, and returns it.
  - `void RecordPresentPassStart(VkCommandBuffer cmd, std::uint32_t
    frameInFlightIndex) noexcept` / `RecordPresentPassEnd(...)` — same
    shape (and same `IsSupported()`/`m_captureEnabled` guard) as the
    offscreen pair, but indexed via `PresentTimestampSlotBase
    (frameInFlightIndex)` (Phase 4A) rather than a `GpuTimingSlot`.
  - `GpuTimingSample ReadPresentResultIfAvailable(std::uint32_t
    frameInFlightIndex) noexcept` — same `IsSupported()`/
    `m_captureEnabled` guards as `ReadOffscreenResultNow()` above (in that
    order: `Unsupported` wins over `Absent` if somehow both would apply,
    since "can never work" is the more specific/permanent fact). If
    supported and capture-enabled but
    `!m_presentSlotEverWritten[frameInFlightIndex]`, returns (and caches)
    `GpuTimingSample{ Status::Absent, 0.0 }` WITHOUT reading the pool at
    all (this is the deliberate warm-up-frame handling — see Phase 4D's
    own detailed sequencing for exactly why this flag, and not a
    frame-count heuristic, is the correct condition). Otherwise reads/
    converts/caches/returns a `Status::Present` sample exactly like the
    offscreen version.
  - `void MarkPresentSlotWritten(std::uint32_t frameInFlightIndex)
    ...
    real frame with capture enabled — NOT on a minimized/pending-resize
    early return, and **[v2]** not while `m_captureEnabled` is false
    either, since no write actually happened in that case; guard this
    method the same way as the two `Record*` methods above so a caller
    that always calls it unconditionally right after them stays correct
    without needing its own separate `if`).
...
- `Renderer` gains `GpuTimingSample LastGpuTiming(GpuTimingSlot slot)
  ...
  across two.
- **[v2]** `Renderer` also gains `void SetGpuTimingCaptureEnabled(bool
  enabled) noexcept { m_gpuTiming->SetCaptureEnabled(enabled); }` — landed
  in this same sub-phase, for the same "review the whole public surface
  change together" reason, even though `Application.cpp` doesn't actually
  start calling it until this sub-phase's own done-when checklist below
  (the class exists, the toggle exists and is provably wired, before any
  real per-frame recording exists to gate).
...
- No change to `Renderer::RenderOffscreen()`'s signature yet (the new
  `std::optional<GpuTimingSlot>` parameter is Phase 4C's addition, since
  it has no purpose until something actually consumes it).
- No `FrameProfiler::SetGpuPassTiming()` call anywhere yet.
- **[v2]** `Application::Run()` does not yet call
  `Renderer::SetGpuTimingCaptureEnabled()` in production code — wiring the
  METHOD in this sub-phase (so its existence/correctness is reviewed
  alongside the rest of the ownership graph) is separate from wiring the
  CALL SITE, which naturally lands in Phase 4C/4D alongside the first real
  recording code it's meant to gate (there is nothing to gate before
  then).
...
- **[v2]** Additionally verify, manually, that a build configured with
  `-DGTE_ENABLE_PROFILER=OFF` creates ZERO `VkQueryPool`s at all — e.g. by
  a temporary log statement inside `GpuTimingService`'s constructor
  confirming the `#if !GTE_ENABLE_PROFILER` branch is taken and
  `m_queryPool` stays `std::nullopt`, or by confirming with a validation-
  layer object-tracking dump that no `VK_OBJECT_TYPE_QUERY_POOL` handle
  exists in that configuration. This is the Tier 2 verification for this
  sub-phase's new compile-time gate; the Tier 1 half (does
  `SetCaptureEnabled(false)` actually suppress the record/read methods'
  effects) is covered below.
- What IS Tier-1-testable here and must be tested as such: the internal
  ...
  allows it" rule. **[v2]** This includes the tri-state PRIORITY logic
  `Read*` methods use (`Unsupported` wins over `Absent` wins over
  `Present`, in that specific order, depending on `IsSupported()`/
  `m_captureEnabled`/warm-up-flag) — if this can be pulled out as a small,
  pure `ResolveGpuTimingStatus(bool supported, bool captureEnabled, bool
  hasWrittenData) noexcept -> GpuTimingSample::Status` function that a test
  can call directly with all 8 combinations of its 3 booleans, do so; this
  is exactly the kind of "small decision table with several boolean
  inputs" logic that is trivially Tier-1-testable in isolation even though
  the class hosting it is otherwise Tier 2.
...
warnings; a `GTE_ENABLE_PROFILER=OFF` build creates no `VkQueryPool` at
all; nothing about the Profiler panel has changed (still shows `N/A` for
GPU Timing, same as before this sub-phase, for the same "nothing reports
to it yet" reason).
...
**1. `Renderer::RenderOffscreen()` gains a `std::optional<GpuTimingSlot>`
parameter:**
...
intent explicitly is the safer choice here). **[v2] `std::nullopt` is
just as explicit a choice as any `GpuTimingSlot` value — it is NOT a
default, it must be spelled out at every call site — it simply means "this
particular call has nothing to do with the Profiler's three named passes,
don't touch a query slot for it."**

**Existing call sites that must be updated:**
...
- **[v2] `src/Editor/AssetPreviewMesh.cpp`'s `RenderOffscreen()` call
  (confirmed to exist — see `README.md`, "Editor / Debug UI": "records its
  own indexed draw call via a `Renderer::RenderOffscreen()` `recordExtra`
  callback") becomes `m_renderer.RenderOffscreen(target, std::nullopt,
  recordExtra)`.** This is a Profiler-invisible, on-demand Inspector
  preview render — it is not one of the three named `GpuPass` values, has
  no business occupying either offscreen query slot, and must never be
  confused with (or silently overwrite the cached timing of) "Game View"/
  "Scene View." This is the exact bug this revision's changelog entry #2
  exists to prevent.
- **[v2] `src/Editor/BoneViewerWindow.cpp`, if (as strongly suspected — it
  reuses `AssetPreviewMesh`'s own shader pair and rendering approach, per
  `README.md`) it also calls `Renderer::RenderOffscreen()` directly, must
  be updated the same way, with `std::nullopt`.** Confirm this by grep at
  implementation time (`grep -rn "RenderOffscreen(" src/`) — the exact
  same "every call site must compile" forcing function v1 already relied
  on for completeness, now made more specific: every non-Application.cpp
  call site found by that grep should be passing `std::nullopt`, and any
  call site found passing `GpuTimingSlot::Offscreen0`/`Offscreen1` outside
  of `Application.cpp`'s own Game/Scene blocks should be treated as a bug
  to fix before this sub-phase is considered done, not shipped as-is.
- Any FURTHER `RenderOffscreen()` call site turned up by that same grep
  (this document cannot promise the two named above are the only ones —
  only that they are the two confirmed via direct reading of `README.md`
  at the time this revision was written) must be updated too, or the
  build simply won't compile — a good, cheap forcing function that
  guarantees nothing is missed, now with a clear, correct default answer
  (`std::nullopt`) for "which slot does an unfamiliar call site need,"
  rather than v1's ambiguous "must be updated too, to be confirmed once
...
2. **NEW:** `if (timingSlot.has_value()) { m_gpuTiming->
   RecordOffscreenPassStart(m_offscreenCommandBuffer, *timingSlot); }` —
   records the reset (for BOTH this pass's start/end slots) plus the start
   timestamp, into the SAME command buffer about to record the actual
   pass, but ONLY when the caller actually asked to be timed. Safe to do
   here specifically because step 1's existing `vkWaitForFences()` already
   proved the LAST use of this exact slot pair (last frame's call for this
   SAME logical pass) is complete. **[v2]** When `timingSlot` is
   `std::nullopt` (e.g. an `AssetPreviewMesh` call), this whole step is
   skipped entirely — no Vulkan call, no slot touched, exactly as if this
   phase didn't exist for that call.
3. *(existing, unchanged)* `frameRecorder.RecordFrame(m_offscreenCommandBuffer,
   ...
   between the two new timestamp writes when present, with no changes to
   this call itself at all.
4. **NEW:** `if (timingSlot.has_value()) { m_gpuTiming->
   RecordOffscreenPassEnd(m_offscreenCommandBuffer, *timingSlot); }`
...
6. **NEW, inserted immediately after that final existing wait, still
   ...
   `if (timingSlot.has_value()) { m_gpuTiming->ReadOffscreenResultNow(
   *timingSlot); }` — safe precisely because the wait in step 5 already
   guarantees this exact submission (including this exact pass's
   timestamp writes) has completed. Skipped entirely when `timingSlot` is
   `std::nullopt` — there is nothing to read back, and no cache entry to
   update.
...
**5. Handling an unsupported device (or a `GTE_ENABLE_PROFILER=OFF`
build, or Capture switched off).** `GpuTimingService::
ReadOffscreenResultNow()` (Phase 4B) already returns `Status::Unsupported`
without touching Vulkan when `!IsSupported()`, and (**[v2]**)
`Status::Absent` without touching Vulkan when capture is currently
disabled — `ToProfilingGpuSampleStatus()` maps both straight to their
`Profiling::GpuSampleStatus` equivalents, and the Profiler panel already
renders `Unsupported` distinctly from `Absent`/`N/A` (confirmed:
`ProfilerPanelData.h`'s `FormatGpuTimingLine()`, per
...
**6. [v2] Wiring the runtime capture gate's call site.** `Application::
Run()` gains one new line, once per frame, before the first
`RenderOffscreen()`/`Present()` call of that frame (the exact placement
inside the function matters less than "before any rendering this frame" —
recommend right next to the existing `BeginFrame()` call, since both are
"per-frame setup" in spirit):
```
m_renderer.SetGpuTimingCaptureEnabled(Profiling::FrameProfiler::Instance().IsCaptureEnabled());
```
This is what makes the Editor's existing "Capture" checkbox (Phase 7,
`ProfilerPanel.cpp`) also genuinely gate this phase's Vulkan work, not
merely the ring-buffer bookkeeping it already gated before this phase
existed — see Step 2.3's own constraint and Phase 4B's design for the
full reasoning.
...
- **[v2]** With "Game"/"Scene" both visible and animating, select a Mesh
  asset in "Project" (triggering `AssetPreviewMesh`'s own
  `RenderOffscreen()` call every frame while the Inspector shows it), and
  confirm Game View/Scene View's GPU Timing numbers are completely
  unaffected (no glitch, no freeze, no sudden jump) by the preview opening,
  changing, or closing — the concrete regression test for changelog
  entry #2's fix. Repeat with the Bone Viewer open instead, if it turns
  out to also call `RenderOffscreen()`.
- **[v2]** Untick the Editor's "Capture" checkbox, confirm (via RenderDoc
  or a validation-layer capture) that no new `vkCmdResetQueryPool`/
  `vkCmdWriteTimestamp2` commands appear in the offscreen command
  buffer(s) while it stays unticked, and confirm the Profiler panel's
  Game/Scene GPU Timing lines read `N/A` (not a frozen last-known value)
  the whole time it's off.

**Done when:** the Profiler panel shows real GPU milliseconds for
whichever of "Game"/"Scene" is currently visible, hidden panels correctly
show `N/A` (never `0.00 ms`), and neither `AssetPreviewMesh` nor the Bone
Viewer's own `RenderOffscreen()` calls have any observable effect on
either value — exactly the `Phase4.md`-specified "Done when" condition for
this chunk, now delivered with the full mechanical detail above plus the
two gaps this revision closes.
...
3. **NEW, inserted immediately after that wait (i.e. BEFORE
   ...
   synchronization fact it depends on):** `m_gpuTiming->
   ReadPresentResultIfAvailable(m_currentFrame);` — this call's return
   value doesn't even need to be kept as a local; per the API design in
   this plan (`Renderer::LastGpuTiming()` is a SEPARATE pull accessor, not
   bundled into `Present()`'s return value), the result is ALREADY cached
   into `m_gpuTiming`'s own internal state as this call's side effect,
   regardless of what happens afterward in this function. This is
   EXACTLY why the "pull accessor" API shape was chosen over trying to
   bundle GPU timing into `Present()`'s existing `std::optional<DrawStats>`
   return value: `Present()` has several more existing early-return paths
   between here and its normal end (the `VK_ERROR_OUT_OF_DATE_KHR`
   acquire-failure path in particular), and a return-value-based design
   would have to thread this already-computed data through every one of
   them just to not lose it — the cache-as-a-side-effect design means none
   of that plumbing is needed at all. See this document's own "Overall API
   surface" section above, and the design-decision log below.
...
5. **NEW:** `m_gpuTiming->RecordPresentPassStart(cmd, m_currentFrame);`
   ...
   pair) plus the start timestamp (a no-op if unsupported/uncompiled-in/
   capture-disabled — see Phase 4B). Safe here because step 2's fence wait
...
7. **NEW:** `m_gpuTiming->RecordPresentPassEnd(cmd, m_currentFrame);`
   ...
   warm-up-tracking flag — itself a no-op unless a real write actually
   happened this call, per Phase 4B's updated design, so it's safe to call
...
...`, the FIRST call to `Present()` that actually records a frame (with
capture enabled — see below) writes into slot-pair 0 (and sets
`m_presentSlotEverWritten[0] = true`). The SECOND such call writes into
slot-pair 1 (sets `[1] = true`). The THIRD such call reuses slot-pair 0 —
at this point, step 2's fence wait for `fence[0]` is waiting on exactly
the FIRST call's own submission, which by now is virtually certain to
have long since completed (a whole additional frame's worth of GPU/CPU
work happened in between) — so by the time the THIRD call reaches step 3,
`m_presentSlotEverWritten[0]` is already `true` and the read is both SAFE
and genuinely MEANINGFUL (real data from the first call's Present pass).
Only the very FIRST and SECOND calls ever see their own slot's flag still
`false` — exactly two "no data yet" frames at the very start of a session,
correctly reported as `Absent`, never a crash/garbage read from an
un-written query. **A frame-count-based heuristic (e.g. "skip if
`CompletedFrameCount() < 2`") would be WRONG here** — it conflates "how
many frames the whole engine has run" with "how many times THIS SPECIFIC
swapchain's `Present()` has actually recorded a real frame WITH CAPTURE
ENABLED," which can diverge in at least two ways: (a) a
resize/minimize/restore cycle where several `BeginFrame()`/`EndFrame()`
pairs happen while `Present()` itself keeps early-returning
`std::nullopt`, and (**[v2]**) (b) capture being toggled off for a while
and back on — `m_presentSlotEverWritten` correctly stays exactly as it was
across a capture-disabled stretch (no write happens, so the flag is never
touched, one way or the other, by `RecordPresentPassStart()`/`End()`'s own
now-conditional no-op), so re-enabling capture simply resumes exactly
where it left off with no fake "re-warm-up" period — the per-slot boolean
flag is the only condition that is correct BY CONSTRUCTION regardless of
how many (if any) early-returns or capture-disabled frames happened in
between.
...
- **[v2]** Untick "Capture" mid-session, wait several frames, re-tick it,
  and confirm Present's GPU Timing correctly shows `Absent` throughout the
  disabled stretch and resumes real values within at most 2 frames of
  re-enabling — the concrete regression check for the warm-up-flag
  interaction with capture-toggling described in point 2 above.
...
- **GPU timing is reported via a Renderer-local `GpuTimingSample`/
  ...
  consistently here, and **[v2]** applied again in the OTHER direction for
  the runtime capture-enabled bridge (`Profiling::FrameProfiler::
  IsCaptureEnabled()` → `Renderer::SetGpuTimingCaptureEnabled(bool)` — a
  plain `bool`, never a `Profiling::`-typed value crossing into
  `Renderer`). This keeps `Renderer` exactly as decoupled from the
  Profiling module as it is from ECS today.
- **`Renderer::RenderOffscreen()`'s new `std::optional<GpuTimingSlot>`
  parameter has NO default value, and `std::nullopt` is one of its two
  equally explicit possible meanings, not a fallback.** Every caller must
  state whether it wants Profiler-visible GPU timing at all, and if so,
  for which of the two offscreen slots — a silent default would risk two
  unrelated callers accidentally sharing cached GPU-timing data with no
  compiler error to catch it (this was v1's own reasoning); **[v2]**
  `std::optional` specifically (rather than a bare, no-default
  `GpuTimingSlot`) exists because v1's original design had exactly this
  same silent-collision risk for a DIFFERENT reason than the one it
  guarded against — a caller like `AssetPreviewMesh` that has nothing to
  do with either named offscreen pass would otherwise have been forced to
  pick one anyway, just to compile, recreating the exact bug the missing
  default was meant to prevent. `std::nullopt` closes that gap the same
  way the missing default closes the original one: by making the caller's
  intent explicit and compiler-checked either way.
...
- **A per-slot boolean "has this ever been written" flag, not a global
  ...
  editor who doesn't re-read that reasoning first, especially now that
  capture-toggling (below) is a second, independent reason the flag can
  legitimately stay unset for a stretch of frames.
...
- **The whole 8-slot query pool is created ONCE, sized for exactly
  ...
  thing that ever changes its validity is device support itself (queried
  once at construction and never re-checked afterward, since Vulkan
  device capabilities do not change at runtime) or, **[v2]**, the
  `GTE_ENABLE_PROFILER` compile-time switch, which is likewise fixed for
  the entire lifetime of one built executable.
- **[v2] A non-Profiler-tracked `RenderOffscreen()` caller (e.g.
  `AssetPreviewMesh`, the Bone Viewer) opts OUT via `std::nullopt`,
  rather than being assigned a THIRD `GpuTimingSlot` value.** A dedicated
  "misc"/"untracked" slot was considered and rejected: it would still
  consume a query-pool slot pair, still incur a (small but real)
  reset+write+read cost for a render nobody asked to have timed, and would
  still need every such caller to remember to pass the SAME sentinel
  value rather than accidentally reusing `Offscreen0`/`Offscreen1` — the
  exact mistake this fix exists to prevent, just moved one level down.
  `std::optional<GpuTimingSlot>` communicates "untracked" at the TYPE
  level instead: there is no `GpuTimingSlot` value meaning "none," so a
  caller genuinely cannot pass one by accident; they either name a real
  slot or explicitly opt out, and the compiler enforces that they pick one
  of the two.
- **[v2] `GpuTimingService`'s actual Vulkan work is gated by BOTH a
  compile-time switch (`GTE_ENABLE_PROFILER`, decided once, forever, per
  built executable) and a runtime switch (`SetCaptureEnabled()`, driven
  every frame from `FrameProfiler::IsCaptureEnabled()`), mirroring the
  exact two-layer design AGENTS.md's "Profiling" section already
  documents for `ScopeTimer`.** This was the single biggest gap in v1 (see
  this document's own Changelog) — DrawStats' own reasoning for NOT
  needing this same gate (it rides for free on an already-necessary
  per-frame iteration) does not transfer to a genuinely new, non-free
  per-frame GPU/driver cost like a timestamp query. The bridge from
  `Profiling::FrameProfiler` (runtime layer only — the compile-time layer
  needs no bridge, it's resolved once at compile time inside
  `GpuTimingService` itself) into `Renderer` stays a plain `bool`, keeping
  `Renderer` exactly as free of `Profiling/` headers as this document's
  pre-existing design decisions already require.
...
- **No GPU-pass attribution for a secondary ImGui platform window**
  ...
  Step 4 refusal list.
- **[v2] No GPU-pass attribution — of any kind, tracked or untracked —
  for the Editor's own debug/preview rendering** (`AssetPreviewMesh`'s
  Inspector mesh preview, the Bone Viewer's own viewport). These calls
  keep rendering exactly as they do today; they simply pass
  `std::nullopt` to `RenderOffscreen()`'s new parameter and are otherwise
  entirely untouched by this phase. They are not, and should not become,
  a fourth/fifth named `GpuPass` — the Profiler's fixed three-pass model
  (`GameView`/`SceneView`/`Present`) is deliberately unchanged by this
  phase (see `ProfilingTypes.h`'s own `kGpuPassCount = 3`), and extending
  it to cover ad hoc Editor debug tooling is out of scope here.
...
- **No Editor/UI work of any kind.** The Profiler panel's "GPU Timing"
  ...
  single ImGui call. **[v2]** The new `Renderer::SetGpuTimingCaptureEnabled()`
  bridge reuses the EXISTING "Capture" checkbox (already shipped in Phase
  7) rather than adding a new one — no new control is added to the panel.
...
2. Re-read the actual current source of every file named in Step 2.1/2.3
   ...
   `DrawStats.h`, `Application.cpp`, **[v2]** `AssetPreviewMesh.cpp` and
   `BoneViewerWindow.cpp`) — this document was written from a real reading
   of that exact code, and it may have changed since; if anything here
   disagrees with what the source actually says by the time
   ...
   In particular, confirm by direct grep (`grep -rn "RenderOffscreen("
   src/`) exactly which files call `Renderer::RenderOffscreen()` before
   starting Phase 4C — this document names `AssetPreviewMesh.cpp` with
   confidence (confirmed against `README.md`'s own text) and
   `BoneViewerWindow.cpp` as a strong suspicion, not a confirmed fact.
...
- [ ] Nothing under `src/Renderer/` (specifically `GpuTiming.h/.cpp`,
      ...
      `SetGpuPassTiming()`/`FrameProfiler::IsCaptureEnabled()` lives ONLY in
      `Application.cpp`.
...
- [ ] **[v2]** Every call site of `Renderer::RenderOffscreen()` other than
      `Application.cpp`'s Game/Scene blocks passes `std::nullopt` for the
      new timing-slot parameter — verified by grep, not by assumption —
      and a manual check confirms none of those calls perturb the
      Profiler's Game View/Scene View GPU Timing values.
- [ ] **[v2]** `Application::Run()` calls
      `Renderer::SetGpuTimingCaptureEnabled(Profiling::FrameProfiler::
      Instance().IsCaptureEnabled())` once per frame, and toggling the
      Editor's "Capture" checkbox off is manually verified (via RenderDoc
      or the validation layer) to stop new `vkCmdResetQueryPool`/
      `vkCmdWriteTimestamp2` commands from appearing, not just to stop the
      Profiler panel's own display from updating.
- [ ] **[v2]** A `-DGTE_ENABLE_PROFILER=OFF` build is confirmed (manually,
      via a validation-layer object-tracking dump or a temporary log
      statement) to never create a `VkQueryPool` for this feature at all.
...
- [ ] `AGENTS.md`'s "Profiling" section is updated once this phase lands
      ...
      section was updated for Phase 3/5's own landings, and to document
      the new `GTE_ENABLE_PROFILER`/`SetCaptureEnabled()` gating this
      feature respects, alongside the existing `ScopeTimer` description of
      the same two-layer switch.
...
resize/minimize/capture-toggle sequence not anticipated here, or
similar), **the correct response is NOT to block or revert 4A/4B/4C** —
ship those three as a complete, honest, partial win (Present's GPU Timing
simply stays at its pre-Phase-4 "N/A" state a while longer, which the
Profiler panel already renders correctly and is not a regression from
today), log the specific blocker encountered in this document's own
changelog (extend the "Changelog: v1 -> v2" section at the top of this
file into a `v3` entry the next time this document is revised), and
revisit 4D as its own, later, dedicated follow-up. This mirrors
...
verification performed — **[v2]** including the `AssetPreviewMesh`/Bone
Viewer isolation check and the capture-toggle verification, both new to
this revision) every prior phase's own landing entry in that document
already demonstrates. At that point, of `PROFILER_STRATEGY_v2.md`'s
