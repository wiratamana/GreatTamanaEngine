...
AssetImportCommandBridge::SubmitResult AssetImportCommandBridge::SubmitAndWait(
    AssetImportCommandRequest request, int timeoutMilliseconds)
{
...
bool AssetImportCommandBridge::IsCommandPending() const
{
    std::lock_guard<std::mutex> lock(m_mutex);
    return m_requested;
}

std::optional<AssetImportCommandRequest> AssetImportCommandBridge::TryPeekPendingCommandRequest() const
{
    std::lock_guard<std::mutex> lock(m_mutex);
    if (!m_requested) {
        return std::nullopt;
...
void AssetImportCommandBridge::FulfillCommand(AssetImportCommandResult result)
{
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        if (!m_requested) {
