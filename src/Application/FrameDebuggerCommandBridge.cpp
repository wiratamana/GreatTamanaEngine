...
FrameDebuggerCommandBridge::SubmitResult FrameDebuggerCommandBridge::SubmitAndWait(
    FrameDebuggerCommandRequest request, int timeoutMilliseconds)
{
...
bool FrameDebuggerCommandBridge::IsCommandPending() const
{
    std::lock_guard<std::mutex> lock(m_mutex);
    return m_requested;
}

std::optional<FrameDebuggerCommandRequest> FrameDebuggerCommandBridge::TryPeekPendingCommandRequest() const
{
    std::lock_guard<std::mutex> lock(m_mutex);
    if (!m_requested) {
        return std::nullopt;
...
void FrameDebuggerCommandBridge::FulfillCommand(FrameDebuggerCommandResult result)
{
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        if (!m_requested) {
