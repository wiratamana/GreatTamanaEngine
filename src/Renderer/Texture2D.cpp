...
Texture2D::Texture2D(
    ...
    , m_device(device)
    // Clamp to at least 1x1 - same defensive reasoning as RenderTexture::
    // Create(): a zero-sized VkImage is invalid, and a caller could in
    // principle hand in a degenerate width/height (e.g. a corrupt image
    ...
    , m_height(height > 0 ? height : 1)
{
    ...
    // TRANSFER_DST so GpuResourceFactory::CreateTexture2D() can copy staged
    ...
    if (vmaCreateImage(m_allocator, &imageInfo, &allocCreateInfo, &m_image, &m_allocation, &allocationInfo) !=
        VK_SUCCESS) {
        throw std::runtime_error("Texture2D: vmaCreateImage failed.");
    }

    // Registers this exact allocation - see AGENTS.md ("GPU resource memory
    // tracking"). A Texture2D is never resized/recreated in place (see the
    // class comment), so unlike RenderTexture there is no re-tracking path
    // to worry about here.
    ...
    if (vkCreateImageView(m_device, &viewInfo, nullptr, &m_imageView) != VK_SUCCESS) {
        ...
        throw std::runtime_error("Texture2D: vkCreateImageView failed.");
    }
    ...
    if (vkCreateSampler(m_device, &samplerInfo, nullptr, &m_sampler) != VK_SUCCESS) {
        ...
        m_tracker->Untrack(m_handle);
        vmaDestroyImage(m_allocator, m_image, m_allocation);
        throw std::runtime_error("Texture2D: vkCreateSampler failed.");
    }
}
